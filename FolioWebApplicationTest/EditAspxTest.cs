...
namespace FolioWebApplicationTest
{
    ...
    public class EditAspxTest
    {
        ...
        [TestMethod]
        public void OrganizationType2sEditAspxTest()
        {
            var s = Stopwatch.StartNew();
            try
            {
                var ot2 = folioServiceContext.OrganizationType2s(take: 1).SingleOrDefault();
                if (ot2 != null)
                {
                    var hrm = httpClient.GetAsync($"{Url}/OrganizationType2s/Edit.aspx?Id={ot2.Id}").Result;
                    hrm.EnsureSuccessStatusCode();
                    Assert.IsTrue(s.Elapsed < timeSpan);
                }
                else
            ...
            {
                traceSource.TraceEvent(TraceEventType.Information, 0, $"OrganizationType2sEditAspxTest()\r\n    ElapsedTime={s.Elapsed}");
            }
        }
