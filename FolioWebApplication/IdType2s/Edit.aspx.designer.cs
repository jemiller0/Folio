using System.Web.UI.WebControls;
using Telerik.Web.UI;

namespace FolioWebApplication.IdType2s
{
    public partial class Edit
    {
        protected Panel IdType2Panel;

        protected FormView IdType2FormView;

        protected Panel ActualCostRecordIdentifiersPanel;

        protected RadGrid ActualCostRecordIdentifiersRadGrid;

        protected Panel IdentifiersPanel;

        protected RadGrid IdentifiersRadGrid;

        protected Panel InvoiceItem2sPanel;

        protected RadGrid InvoiceItem2sRadGrid;

        protected Panel OrderItemProductIdsPanel;

        protected RadGrid OrderItemProductIdsRadGrid;

        protected Panel PrecedingSucceedingTitleIdentifiersPanel;

        protected RadGrid PrecedingSucceedingTitleIdentifiersRadGrid;

        protected Panel RequestIdentifiersPanel;

        protected RadGrid RequestIdentifiersRadGrid;

        protected Panel TitleProductIdsPanel;

        protected RadGrid TitleProductIdsRadGrid;

        protected RadAjaxManager RadAjaxManager1;
    }
}
