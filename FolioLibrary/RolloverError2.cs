...
{
    // uc.rollover_errors -> uchicago_mod_finance_storage.ledger_fiscal_year_rollover_error
    // RolloverError2 -> RolloverError
    [DisplayColumn(nameof(Id)), DisplayName("Rollover Errors"), JsonConverter(typeof(JsonPathJsonConverter<RolloverError2>)), JsonObject(MemberSerialization = MemberSerialization.OptIn), Table("rollover_errors", Schema = "uc")]
    public partial class RolloverError2
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.RolloverError.json")))
            {
        ...
        [Column("rollover_id"), Display(Name = "Rollover", Order = 3), JsonProperty("ledgerRolloverId")]
        ...
        [Display(Name = "Creation User", Order = 8), InverseProperty("RolloverError2s")]
        ...
        [Display(Name = "Last Write User", Order = 12), InverseProperty("RolloverError2s1")]
        ...
        [Column("content"), CustomValidation(typeof(RolloverError), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 15), Editable(false)]
        public virtual string Content { get; set; }

        public override string ToString() => $"{{ {nameof(Id)} = {Id}, {nameof(RolloverId)} = {RolloverId}, {nameof(ErrorType)} = {ErrorType}, {nameof(FailedAction)} = {FailedAction}, {nameof(ErrorMessage)} = {ErrorMessage}, {nameof(CreationTime)} = {CreationTime}, {nameof(CreationUserId)} = {CreationUserId}, {nameof(CreationUserUsername)} = {CreationUserUsername}, {nameof(LastWriteTime)} = {LastWriteTime}, {nameof(LastWriteUserId)} = {LastWriteUserId}, {nameof(LastWriteUserUsername)} = {LastWriteUserUsername}, {nameof(Content)} = {Content} }}";

        public static RolloverError2 FromJObject(JObject jObject) => jObject != null ? new RolloverError2
        {
