...
{
    [Table("staff_slips", Schema = "diku_mod_circulation_storage")]
    public partial class StaffSlip
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StringReader(value))
            using (var jtr = new JsonTextReader(sr))
            using (var sr2 = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.StaffSlip.json")))
        ...
        [Column("jsonb"), CustomValidation(typeof(StaffSlip), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 2), Required]
