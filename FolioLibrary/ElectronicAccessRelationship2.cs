...
{
    // uc.electronic_access_relationships -> uchicago_mod_inventory_storage.electronic_access_relationship
    // ElectronicAccessRelationship2 -> ElectronicAccessRelationship
    [DisplayColumn(nameof(Name)), DisplayName("Electronic Access Relationships"), JsonConverter(typeof(JsonPathJsonConverter<ElectronicAccessRelationship2>)), JsonObject(MemberSerialization = MemberSerialization.OptIn), Table("electronic_access_relationships", Schema = "uc")]
    public partial class ElectronicAccessRelationship2
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.ElectronicAccessRelationship.json")))
            {
        ...
        [Display(Name = "Creation User", Order = 5), InverseProperty("ElectronicAccessRelationship2s")]
        ...
        [Display(Name = "Last Write User", Order = 9), InverseProperty("ElectronicAccessRelationship2s1")]
        ...
        [Column("content"), CustomValidation(typeof(ElectronicAccessRelationship), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 12), Editable(false)]
        public virtual string Content { get; set; }

        [Display(Name = "Electronic Accesses", Order = 13)]
        public virtual ICollection<ElectronicAccess> ElectronicAccesses { get; set; }

        [Display(Name = "Holding Electronic Accesses", Order = 14)]
        public virtual ICollection<HoldingElectronicAccess> HoldingElectronicAccesses { get; set; }

        [Display(Name = "Item Electronic Accesses", Order = 15)]
        ...
        public static ElectronicAccessRelationship2 FromJObject(JObject jObject) => jObject != null ? new ElectronicAccessRelationship2
        {
