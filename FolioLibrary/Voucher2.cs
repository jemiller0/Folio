...
{
    // uc.vouchers -> uchicago_mod_invoice_storage.vouchers
    // Voucher2 -> Voucher
    ...
    public partial class Voucher2
    {
        ...
        public static Voucher2 FromJObject(JObject jObject) => jObject != null ? new Voucher2
        {
            ...
            VoucherDate = ((DateTime?)jObject.SelectToken("voucherDate"))?.ToUniversalTime(),
            ...
            VoucherAcquisitionsUnits = jObject.SelectToken("acqUnitIds")?.Select(jt => VoucherAcquisitionsUnit.FromJObject((JValue)jt)).ToArray()
        } : null;
