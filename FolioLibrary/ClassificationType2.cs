...
{
    // uc.classification_types -> uchicago_mod_inventory_storage.classification_type
    // ClassificationType2 -> ClassificationType
    [DisplayColumn(nameof(Name)), DisplayName("Classification Types"), JsonConverter(typeof(JsonPathJsonConverter<ClassificationType2>)), JsonObject(MemberSerialization = MemberSerialization.OptIn), Table("classification_types", Schema = "uc")]
    public partial class ClassificationType2
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.ClassificationType.json")))
            {
        ...
        [Display(Name = "Creation User", Order = 5), InverseProperty("ClassificationType2s")]
        ...
        [Display(Name = "Last Write User", Order = 9), InverseProperty("ClassificationType2s1")]
        ...
        [Column("content"), CustomValidation(typeof(ClassificationType), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 12), Editable(false)]
        ...
        public static ClassificationType2 FromJObject(JObject jObject) => jObject != null ? new ClassificationType2
        {
