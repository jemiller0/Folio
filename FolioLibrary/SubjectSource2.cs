...
{
    // uc.subject_sources -> uchicago_mod_inventory_storage.subject_source
    // SubjectSource2 -> SubjectSource
    [DisplayColumn(nameof(Name)), DisplayName("Subject Sources"), JsonConverter(typeof(JsonPathJsonConverter<SubjectSource2>)), JsonObject(MemberSerialization = MemberSerialization.OptIn), Table("subject_sources", Schema = "uc")]
    public partial class SubjectSource2
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.SubjectSource.json")))
            {
        ...
        [Column("source"), Display(Order = 4), JsonProperty("source"), RegularExpression(@"^(folio|local|consortium)$"), StringLength(1024)]
        ...
        [Display(Name = "Creation User", Order = 6), InverseProperty("SubjectSource2s")]
        ...
        [Display(Name = "Last Write User", Order = 10), InverseProperty("SubjectSource2s1")]
        ...
        [Column("content"), CustomValidation(typeof(SubjectSource), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 13), Editable(false)]
        ...
        public static SubjectSource2 FromJObject(JObject jObject) => jObject != null ? new SubjectSource2
        {
