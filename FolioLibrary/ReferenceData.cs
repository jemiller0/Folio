...
{
    [Table("reference_datas", Schema = "uc_agreements")]
    public partial class ReferenceData
    {
        ...
        [Column("jsonb"), CustomValidation(typeof(ReferenceData), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 2), Required]
        public virtual string Content { get; set; }

        public override string ToString() => $"{{ {nameof(Id)} = {Id}, {nameof(Content)} = {Content} }}";

        public static ReferenceData FromJObject(JObject jObject) => jObject != null ? new ReferenceData
        {
