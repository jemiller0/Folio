...
{
    // uc.subject_types -> uchicago_mod_inventory_storage.subject_type
    // SubjectType2 -> SubjectType
    [DisplayColumn(nameof(Name)), DisplayName("Subject Types"), JsonConverter(typeof(JsonPathJsonConverter<SubjectType2>)), JsonObject(MemberSerialization = MemberSerialization.OptIn), Table("subject_types", Schema = "uc")]
    public partial class SubjectType2
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.SubjectType.json")))
            {
        ...
        [Display(Name = "Creation User", Order = 5), InverseProperty("SubjectType2s")]
        ...
        [Display(Name = "Last Write User", Order = 9), InverseProperty("SubjectType2s1")]
        ...
        [Column("content"), CustomValidation(typeof(SubjectType), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 12), Editable(false)]
        ...
        public static SubjectType2 FromJObject(JObject jObject) => jObject != null ? new SubjectType2
        {
