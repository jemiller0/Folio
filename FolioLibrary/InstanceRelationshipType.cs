...
{
    [Table("instance_relationship_type", Schema = "diku_mod_inventory_storage")]
    public partial class InstanceRelationshipType
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StringReader(value))
            using (var jtr = new JsonTextReader(sr))
            using (var sr2 = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.InstanceRelationshipType.json")))
        ...
        [Column("jsonb"), CustomValidation(typeof(InstanceRelationshipType), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 2), Required]
