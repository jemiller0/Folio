...
namespace FolioLibrary
{
    ...
    public partial class Library2
    {
        ...
        [Display(Name = "Creation User", Order = 8), InverseProperty("Library2s")]
        ...
        [Display(Name = "Last Write User", Order = 12), InverseProperty("Library2s1")]
        ...
        [Column("content"), CustomValidation(typeof(Library), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 15), Editable(false)]
        public virtual string Content { get; set; }

        [Display(Name = "Locations", Order = 16)]
        public virtual ICollection<Location2> Location2s { get; set; }

        public override string ToString() => $"{{ {nameof(Id)} = {Id}, {nameof(Name)} = {Name}, {nameof(Code)} = {Code}, {nameof(CampusId)} = {CampusId}, {nameof(IsShadow)} = {IsShadow}, {nameof(CreationTime)} = {CreationTime}, {nameof(CreationUserId)} = {CreationUserId}, {nameof(CreationUserUsername)} = {CreationUserUsername}, {nameof(LastWriteTime)} = {LastWriteTime}, {nameof(LastWriteUserId)} = {LastWriteUserId}, {nameof(LastWriteUserUsername)} = {LastWriteUserUsername}, {nameof(Content)} = {Content} }}";
