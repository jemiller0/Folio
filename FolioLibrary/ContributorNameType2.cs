...
{
    // uc.contributor_name_types -> diku_mod_inventory_storage.contributor_name_type
    ...
    public partial class ContributorNameType2
    {
        ...
        [Display(Name = "Creation User", Order = 5), InverseProperty("ContributorNameType2s")]
        ...
        [Display(Name = "Last Write User", Order = 9), InverseProperty("ContributorNameType2s1")]
        ...
        [Column("content"), CustomValidation(typeof(ContributorNameType), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 12), Editable(false)]
        public virtual string Content { get; set; }

        [Display(Order = 13)]
        public virtual ICollection<Contributor> Contributors { get; set; }

        [Display(Name = "Order Item Contributors", Order = 14)]
        public virtual ICollection<OrderItemContributor> OrderItemContributors { get; set; }

        [Display(Name = "Title Contributors", Order = 15)]
        public virtual ICollection<TitleContributor> TitleContributors { get; set; }

        public override string ToString() => $"{{ {nameof(Id)} = {Id}, {nameof(Name)} = {Name}, {nameof(Ordering)} = {Ordering}, {nameof(CreationTime)} = {CreationTime}, {nameof(CreationUserId)} = {CreationUserId}, {nameof(CreationUserUsername)} = {CreationUserUsername}, {nameof(LastWriteTime)} = {LastWriteTime}, {nameof(LastWriteUserId)} = {LastWriteUserId}, {nameof(LastWriteUserUsername)} = {LastWriteUserUsername}, {nameof(Content)} = {Content} }}";
