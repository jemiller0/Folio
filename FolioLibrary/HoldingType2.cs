...
{
    // uc.holding_types -> uchicago_mod_inventory_storage.holdings_type
    // HoldingType2 -> HoldingType
    [DisplayColumn(nameof(Name)), DisplayName("Holding Types"), JsonConverter(typeof(JsonPathJsonConverter<HoldingType2>)), JsonObject(MemberSerialization = MemberSerialization.OptIn), Table("holding_types", Schema = "uc")]
    public partial class HoldingType2
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.HoldingType.json")))
            {
        ...
        [Display(Name = "Creation User", Order = 5), InverseProperty("HoldingType2s")]
        ...
        [Display(Name = "Last Write User", Order = 9), InverseProperty("HoldingType2s1")]
        ...
        [Column("content"), CustomValidation(typeof(HoldingType), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 12), Editable(false)]
        ...
        public static HoldingType2 FromJObject(JObject jObject) => jObject != null ? new HoldingType2
        {
