...
{
    [Table("mode_of_issuance", Schema = "diku_mod_inventory_storage")]
    public partial class ModeOfIssuance
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StringReader(value))
            using (var jtr = new JsonTextReader(sr))
            using (var sr2 = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.ModeOfIssuance.json")))
        ...
        [Column("jsonb"), CustomValidation(typeof(ModeOfIssuance), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 2), Required]
