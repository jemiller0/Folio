...
{
    [Table("auth_attempts", Schema = "diku_mod_login")]
    public partial class AuthAttempt
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StringReader(value))
            using (var jtr = new JsonTextReader(sr))
            using (var sr2 = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.AuthAttempt.json")))
        ...
        [Column("jsonb"), CustomValidation(typeof(AuthAttempt), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 2), Required]
