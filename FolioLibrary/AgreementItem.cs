...
{
    [Table("agreement_items", Schema = "uc_agreements")]
    public partial class AgreementItem
    {
        ...
        [Column("jsonb"), CustomValidation(typeof(AgreementItem), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 2), Required]
        public virtual string Content { get; set; }

        public override string ToString() => $"{{ {nameof(Id)} = {Id}, {nameof(Content)} = {Content} }}";

        public static AgreementItem FromJObject(JObject jObject) => jObject != null ? new AgreementItem
        {
