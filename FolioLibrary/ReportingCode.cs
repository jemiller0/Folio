...
{
    [Table("reporting_code", Schema = "diku_mod_orders_storage")]
    public partial class ReportingCode
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StringReader(value))
            using (var jtr = new JsonTextReader(sr))
            using (var sr2 = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.ReportingCode.json")))
        ...
        [Column("jsonb"), CustomValidation(typeof(ReportingCode), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 2), Required]
