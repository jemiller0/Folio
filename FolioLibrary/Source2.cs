...
{
    // uc.sources -> uchicago_mod_inventory_storage.holdings_records_source
    // Source2 -> Source
    [DisplayColumn(nameof(Name)), DisplayName("Sources"), JsonConverter(typeof(JsonPathJsonConverter<Source2>)), JsonObject(MemberSerialization = MemberSerialization.OptIn), Table("sources", Schema = "uc")]
    public partial class Source2
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.Source.json")))
            {
        ...
        [Display(Name = "Creation User", Order = 5), InverseProperty("Source2s")]
        ...
        [Display(Name = "Last Write User", Order = 9), InverseProperty("Source2s1")]
        ...
        [Column("content"), CustomValidation(typeof(Source), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 12), Editable(false)]
        public virtual string Content { get; set; }

        [Display(Name = "Holdings", Order = 13)]
        public virtual ICollection<Holding2> Holding2s { get; set; }

        [Display(Name = "Journal Records", Order = 14)]
