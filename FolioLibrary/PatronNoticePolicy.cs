...
{
    [Table("patron_notice_policy", Schema = "diku_mod_circulation_storage")]
    public partial class PatronNoticePolicy
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StringReader(value))
            using (var jtr = new JsonTextReader(sr))
            using (var sr2 = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.PatronNoticePolicy.json")))
        ...
        [Column("jsonb"), CustomValidation(typeof(PatronNoticePolicy), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 2), Required]
