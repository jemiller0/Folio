...
namespace FolioLibrary
{
    ...
    public partial class Instance2
    {
        ...
        public static Instance2 FromJObject(JObject jObject) => jObject != null ? new Instance2
        {
            ...
            AlternativeTitles = jObject.SelectToken("alternativeTitles")?.Select(jt => AlternativeTitle.FromJObject((JObject)jt)).ToArray(),
            Classifications = jObject.SelectToken("classifications")?.Select(jt => Classification.FromJObject((JObject)jt)).ToArray(),
            Contributors = jObject.SelectToken("contributors")?.Select(jt => Contributor.FromJObject((JObject)jt)).ToArray(),
            Editions = jObject.SelectToken("editions")?.Select(jt => Edition.FromJObject((JValue)jt)).ToArray(),
            ElectronicAccesses = jObject.SelectToken("electronicAccess")?.Select(jt => ElectronicAccess.FromJObject((JObject)jt)).ToArray(),
            Identifiers = jObject.SelectToken("identifiers")?.Select(jt => Identifier.FromJObject((JObject)jt)).ToArray(),
            ...
            Note2s = jObject.SelectToken("notes")?.Select(jt => Note2.FromJObject((JObject)jt)).ToArray(),
            ...
            Publications = jObject.SelectToken("publication")?.Select(jt => Publication.FromJObject((JObject)jt)).ToArray(),
