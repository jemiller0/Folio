...
namespace FolioLibrary
{
    ...
    public partial class InvoiceItemAdjustmentFund
    {
        ...
        [Display(Name = "Invoice Item Adjustment", Order = 2)]
        public virtual InvoiceItemAdjustment InvoiceItemAdjustment { get; set; }

        [Column("invoice_item_adjustment_id"), Display(Name = "Invoice Item Adjustment", Order = 3), Required, StringLength(1024)]
        ...
        public override string ToString() => $"{{ {nameof(Id)} = {Id}, {nameof(InvoiceItemAdjustmentId)} = {InvoiceItemAdjustmentId}, {nameof(FundCode)} = {FundCode}, {nameof(EncumbranceId)} = {EncumbranceId}, {nameof(FundId)} = {FundId}, {nameof(ExpenseClassId)} = {ExpenseClassId}, {nameof(DistributionType)} = {DistributionType}, {nameof(Value)} = {Value} }}";
