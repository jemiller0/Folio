...
{
    [Table("contributor_name_type", Schema = "diku_mod_inventory_storage")]
    public partial class ContributorNameType
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StringReader(value))
            using (var jtr = new JsonTextReader(sr))
            using (var sr2 = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.ContributorNameType.json")))
        ...
        [Column("jsonb"), CustomValidation(typeof(ContributorNameType), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 2), Required]
