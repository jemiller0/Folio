...
{
    [Table("order_templates", Schema = "uchicago_mod_orders_storage")]
    public partial class OrderTemplate
    {
        ...
        [Column("jsonb"), CustomValidation(typeof(OrderTemplate), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 2), Required]
        public virtual string Content { get; set; }

        public override string ToString() => $"{{ {nameof(Id)} = {Id}, {nameof(Content)} = {Content} }}";

        public static OrderTemplate FromJObject(JObject jObject) => jObject != null ? new OrderTemplate
        {
