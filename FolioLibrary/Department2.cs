...
{
    // uc.departments -> uchicago_mod_users.departments
    // Department2 -> Department
    [DisplayColumn(nameof(Name)), DisplayName("Departments"), JsonConverter(typeof(JsonPathJsonConverter<Department2>)), JsonObject(MemberSerialization = MemberSerialization.OptIn), Table("departments", Schema = "uc")]
    public partial class Department2
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.Department.json")))
            {
        ...
        [Column("source"), Display(Order = 5), JsonProperty("source"), StringLength(1024)]
        ...
        [Display(Name = "Creation User", Order = 7), InverseProperty("Department2s")]
        ...
        [Display(Name = "Last Write User", Order = 11), InverseProperty("Department2s1")]
        ...
        [Column("content"), CustomValidation(typeof(Department), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 14), Editable(false)]
        public virtual string Content { get; set; }

        [Display(Name = "User Departments", Order = 15)]
        public virtual ICollection<UserDepartment> UserDepartments { get; set; }

        public override string ToString() => $"{{ {nameof(Id)} = {Id}, {nameof(Name)} = {Name}, {nameof(Code)} = {Code}, {nameof(UsageNumber)} = {UsageNumber}, {nameof(Source)} = {Source}, {nameof(CreationTime)} = {CreationTime}, {nameof(CreationUserId)} = {CreationUserId}, {nameof(CreationUserUsername)} = {CreationUserUsername}, {nameof(LastWriteTime)} = {LastWriteTime}, {nameof(LastWriteUserId)} = {LastWriteUserId}, {nameof(LastWriteUserUsername)} = {LastWriteUserUsername}, {nameof(Content)} = {Content} }}";

        public static Department2 FromJObject(JObject jObject) => jObject != null ? new Department2
        {
            Id = (Guid?)jObject.SelectToken("id"),
            Name = (string)jObject.SelectToken("name"),
            Code = (string)jObject.SelectToken("code"),
            UsageNumber = (int?)jObject.SelectToken("usageNumber"),
        ...
        public JObject ToJObject() => new JObject(
            ...
            new JProperty("usageNumber", UsageNumber),
