...
{
    [Table("fund_distribution", Schema = "diku_mod_orders_storage")]
    public partial class FundDistribution2
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StringReader(value))
            using (var jtr = new JsonTextReader(sr))
            using (var sr2 = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.FundDistribution2.json")))
        ...
        [Column("jsonb"), CustomValidation(typeof(FundDistribution2), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 2), Required]
