...
{
    [Table("transfer_criteria", Schema = "diku_mod_feesfines")]
    public partial class TransferCriteria
    {
        public static ValidationResult ValidateContent(string value)
        {
            using (var sr = new StringReader(value))
            using (var jtr = new JsonTextReader(sr))
            using (var sr2 = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("FolioLibrary.TransferCriteria.json")))
        ...
        [Column("jsonb"), CustomValidation(typeof(TransferCriteria), nameof(ValidateContent)), DataType(DataType.MultilineText), Display(Order = 2), Required]
