...
{
    // uc.invoice_adjustment_fund_distributions -> uchicago_mod_invoice_storage.invoices
    // InvoiceAdjustmentFund -> Invoice
    [DisplayColumn(nameof(Id)), DisplayName("Invoice Adjustment Funds"), JsonObject(MemberSerialization = MemberSerialization.OptIn), Table("invoice_adjustment_fund_distributions", Schema = "uc")]
    public partial class InvoiceAdjustmentFund
    {
        [Column("id"), ScaffoldColumn(false)]
        public virtual string Id { get; set; }

        [Display(Name = "Invoice Adjustment", Order = 2)]
        public virtual InvoiceAdjustment InvoiceAdjustment { get; set; }

        [Column("invoice_adjustment_id"), Display(Name = "Invoice Adjustment", Order = 3), Required, StringLength(1024)]
        public virtual string InvoiceAdjustmentId { get; set; }
        ...
        public override string ToString() => $"{{ {nameof(Id)} = {Id}, {nameof(InvoiceAdjustmentId)} = {InvoiceAdjustmentId}, {nameof(FundCode)} = {FundCode}, {nameof(EncumbranceId)} = {EncumbranceId}, {nameof(FundId)} = {FundId}, {nameof(ExpenseClassId)} = {ExpenseClassId}, {nameof(DistributionType)} = {DistributionType}, {nameof(Value)} = {Value} }}";

        public static InvoiceAdjustmentFund FromJObject(JObject jObject) => jObject != null ? new InvoiceAdjustmentFund
        {
