...
namespace FolioLibrary
{
    ...
    public partial class InvoiceAdjustmentFund
    {
        ...
        [Column("invoice_adjustment_id"), Display(Name = "Invoice Adjustment Id", Order = 2), Required, StringLength(1024)]
        ...
        public override string ToString() => $"{{ {nameof(Id)} = {Id}, {nameof(InvoiceAdjustmentId)} = {InvoiceAdjustmentId}, {nameof(FundCode)} = {FundCode}, {nameof(EncumbranceId)} = {EncumbranceId}, {nameof(FundId)} = {FundId}, {nameof(InvoiceItemId)} = {InvoiceItemId}, {nameof(DistributionType)} = {DistributionType}, {nameof(ExpenseClassId)} = {ExpenseClassId}, {nameof(Value)} = {Value} }}";
