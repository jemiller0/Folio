...
cf.jsonb#>>'{lookupField,refEntityType}' AS lookup_field_ref_entity_type,
...
l.jsonb#>>'{forUseAtLocation,status}' AS for_use_at_location_status,
uc.DATE_CAST(l.jsonb#>>'{forUseAtLocation,statusDate}') AS for_use_at_location_status_date,
uc.DATE_CAST(l.jsonb#>>'{forUseAtLocation,holdShelfExpirationDate}') AS for_use_at_location_hold_shelf_expiration_date,
...
le.jsonb#>>'{loan,forUseAtLocation,status}' AS loan_for_use_at_location_status,
uc.DATE_CAST(le.jsonb#>>'{loan,forUseAtLocation,statusDate}') AS loan_for_use_at_location_status_date,
uc.DATE_CAST(le.jsonb#>>'{loan,forUseAtLocation,holdShelfExpirationDate}') AS loan_for_use_at_location_hold_shelf_expiration_date,
...
uc.TIMESTAMP_CAST(le.jsonb#>>'{loan,agedToLostDelayedBilling,dateLostItemShouldBeBilled}') AS loan_aged_to_lost_delayed_billing_date_lost_item_should_be_bill,
...
CAST(lp.jsonb#>>'{loansPolicy,forUseAtLocation}' AS BOOLEAN) AS loans_policy_for_use_at_location,
CAST(lp.jsonb#>>'{loansPolicy,holdShelfExpiryPeriodForUseAtLocation,duration}' AS INTEGER) AS loans_policy_hold_shelf_expiry_period_for_use_at_location_durat,
lp.jsonb#>>'{loansPolicy,holdShelfExpiryPeriodForUseAtLocation,intervalId}' AS loans_policy_hold_shelf_expiry_period_for_use_at_location_inter,
...
oi.jsonb#>>'{lastExport,transmissionMethod}' AS last_export_transmission_method,
...
CAST(oot.jsonb AS UUID) AS organization_type_id
...
CREATE VIEW uc.organization_types AS
SELECT
ot.id AS id,
CAST(ot.jsonb->>'_version' AS INTEGER) AS _version,
ot.jsonb->>'name' AS name,
ot.jsonb->>'status' AS status,
uc.TIMESTAMP_CAST(jsonb#>>'{metadata,createdDate}' || CASE WHEN jsonb#>>'{metadata,createdDate}' !~ '([-+]\d\d:\d\d)|Z$' THEN '+00:00' ELSE '' END) AS created_date,
CAST(ot.jsonb#>>'{metadata,createdByUserId}' AS UUID) AS created_by_user_id,
ot.jsonb#>>'{metadata,createdByUsername}' AS created_by_username,
uc.TIMESTAMP_CAST(ot.jsonb#>>'{metadata,updatedDate}') AS updated_date,
CAST(ot.jsonb#>>'{metadata,updatedByUserId}' AS UUID) AS updated_by_user_id,
ot.jsonb#>>'{metadata,updatedByUsername}' AS updated_by_username,
jsonb_pretty(COALESCE(jsonb_set(jsonb, '{metadata,createdDate}', ('"' || (jsonb#>>'{metadata,createdDate}') || CASE WHEN jsonb#>>'{metadata,createdDate}' !~ '([-+]\d\d:\d\d)|Z$' THEN '+00:00' ELSE '' END || '"')::jsonb), jsonb)) AS content
FROM uchicago_mod_organizations_storage.organization_types ot;
...
CAST(r.jsonb->>'sequenceNumber' AS INTEGER) AS sequence_number,
...
CAST(r.jsonb#>>'{item,itemEffectiveLocationId}' AS UUID) AS item_item_effective_location_id,
r.jsonb#>>'{item,itemEffectiveLocationName}' AS item_item_effective_location_name,
CAST(r.jsonb#>>'{item,retrievalServicePointId}' AS UUID) AS item_retrieval_service_point_id,
r.jsonb#>>'{item,retrievalServicePointName}' AS item_retrieval_service_point_name,
...
CAST(r.jsonb->>'isDcbReRequestCancellation' AS BOOLEAN) AS is_dcb_re_request_cancellation,
jsonb_pretty(COALESCE(jsonb_set(jsonb, '{metadata,createdDate}', ('"' || (jsonb#>>'{metadata,createdDate}') || CASE WHEN jsonb#>>'{metadata,createdDate}' !~ '([-+]\d\d:\d\d)|Z$' THEN '+00:00' ELSE '' END || '"')::jsonb), jsonb)) AS content
...
CAST(ss.jsonb->>'isRawHtml' AS BOOLEAN) AS is_raw_html,
...
CAST(t.jsonb->>'nextSequenceNumber' AS INTEGER) AS next_sequence_number,
...
CAST(jsonb#>>'{customFields,dataSource}' AS VARCHAR(1024)) AS source_id,
CAST(jsonb#>>'{customFields,category}' AS VARCHAR(1024)) AS category_code,
CAST(jsonb#>>'{customFields,statCategory}' AS VARCHAR(1024)) AS category_id,
...
CREATE TABLE uc.user_sources (
    ...
    last_write_time TIMESTAMP,
    last_write_username VARCHAR(128),
    CONSTRAINT pk_user_sources PRIMARY KEY(id)
);
INSERT INTO uc.user_sources VALUES
('opt_1', 'Library', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
('opt_2', 'University', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller');
...
('opt_140', 'Visitor', 'VIS', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
('opt_141', 'UC Alum', 'ALU', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
...
('opt_148', 'Arts and Public Life Affiliate', 'ARTPUB', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
...
('opt_142', 'Banned User', 'BANNED', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
...
('opt_14', 'Departmental Academic', 'BLUECARD', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
...
('opt_144', 'Charter School Staff', 'CHRTSTF', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
('opt_138', 'Charter School Student', 'CHRTSTU', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
('opt_21', 'Civic Knowledge Project', 'CKP', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
('opt_146', 'Clancy Library Relocations', 'CLANCY', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
('opt_22', 'Crerar Corp., Non-Profit', 'CORMEMN', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
('opt_23', 'Crerar Corp., Profit', 'COREMEMP', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
('opt_24', 'CRL Staff', 'CRL', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
('opt_147', 'CRSP Staff', 'CRSPSTF', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
...
('opt_145', 'Lumen Christi Staff', 'LUMSTF', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
...
('opt_149', 'Seldon Institute Staff', 'SELDON', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
...
('opt_139', 'Special Arrangement', 'SPAR', CURRENT_TIMESTAMP, 'jemiller', CURRENT_TIMESTAMP, 'jemiller'),
