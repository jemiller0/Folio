...
jsonb->>'hash' AS hash,
jsonb->>'salt' AS salt,
uc.DATE_CAST(jsonb->>'date') AS date,
uc.TIMESTAMP_CAST(jsonb#>>'{metadata,createdDate}' || CASE WHEN jsonb#>>'{metadata,createdDate}' !~ '([-+]\d\d:\d\d)|Z$' THEN '+00:00' ELSE '' END) AS created_date,
CAST(jsonb#>>'{metadata,createdByUserId}' AS UUID) AS created_by_user_id,
jsonb#>>'{metadata,createdByUsername}' AS created_by_username,
uc.TIMESTAMP_CAST(jsonb#>>'{metadata,updatedDate}') AS updated_date,
...
uc.DATE_CAST(jsonb->>'disbursementDate') AS disbursement_date,
...
uc.DATE_CAST(jsonb->>'voucherDate') AS voucher_date,
...
uc.DATE_CAST(jsonb->>'expirationDate') AS expiration_date,
...
uc.DATE_CAST(jsonb#>>'{documentMetadata,metadata,createdDate}') AS document_metadata_metadata_created_date,
...
uc.DATE_CAST(jsonb#>>'{documentMetadata,metadata,updatedDate}') AS document_metadata_metadata_updated_date,
...
uc.DATE_CAST(jsonb->>'dateCreated') AS date_created,
uc.DATE_CAST(jsonb->>'dateUpdated') AS date_updated,
...
jsonb->>'description' AS description,
uc.DATE_CAST(jsonb->>'periodStart') AS period_start,
uc.DATE_CAST(jsonb->>'periodEnd') AS period_end,
...
uc.DATE_CAST(jsonb->>'approvalDate') AS approval_date,
CAST(jsonb->>'batchGroupId' AS UUID) AS batch_group_id,
...
CAST(jsonb->>'exportToAccounting' AS BOOLEAN) AS export_to_accounting,
jsonb->>'folioInvoiceNo' AS folio_invoice_no,
uc.DATE_CAST(jsonb->>'invoiceDate') AS invoice_date,
CAST(jsonb->>'lockTotal' AS DECIMAL(19,2)) AS lock_total,
jsonb->>'note' AS note,
uc.DATE_CAST(jsonb->>'paymentDue') AS payment_due,
uc.DATE_CAST(jsonb->>'paymentDate') AS payment_date,
...
uc.DATE_CAST(jsonb->>'subscriptionStart') AS subscription_start,
uc.DATE_CAST(jsonb->>'subscriptionEnd') AS subscription_end,
CAST(jsonb->>'subTotal' AS DECIMAL(19,2)) AS sub_total,
CAST(jsonb->>'total' AS DECIMAL(19,2)) AS total,
uc.TIMESTAMP_CAST(jsonb#>>'{metadata,createdDate}' || CASE WHEN jsonb#>>'{metadata,createdDate}' !~ '([-+]\d\d:\d\d)|Z$' THEN '+00:00' ELSE '' END) AS created_date,
CAST(jsonb#>>'{metadata,createdByUserId}' AS UUID) AS created_by_user_id,
...
uc.DATE_CAST(jsonb->>'startedDate') AS started_date,
uc.DATE_CAST(jsonb->>'completedDate') AS completed_date,
jsonb->>'status' AS status,
...
uc.DATE_CAST(jsonb->>'declaredLostDate') AS declared_lost_date,
uc.DATE_CAST(jsonb->>'claimedReturnedDate') AS claimed_returned_date,
...
uc.DATE_CAST(jsonb#>>'{agedToLostDelayedBilling,agedToLostDate}') AS aged_to_lost_delayed_billing_aged_to_lost_date,
jsonb_pretty(COALESCE(jsonb_set(jsonb, '{metadata,createdDate}', ('"' || (jsonb#>>'{metadata,createdDate}') || CASE WHEN jsonb#>>'{metadata,createdDate}' !~ '([-+]\d\d:\d\d)|Z$' THEN '+00:00' ELSE '' END || '"')::jsonb), jsonb)) AS content
...
uc.DATE_CAST(jsonb#>>'{loan,dueDate}') AS loan_due_date,
jsonb#>>'{loan,returnDate}' AS loan_return_date,
uc.DATE_CAST(jsonb#>>'{loan,systemReturnDate}') AS loan_system_return_date,
...
uc.DATE_CAST(jsonb#>>'{loan,declaredLostDate}') AS loan_declared_lost_date,
uc.DATE_CAST(jsonb#>>'{loan,claimedReturnedDate}') AS loan_claimed_returned_date,
...
uc.DATE_CAST(jsonb#>>'{loan,metadata,createdDate}') AS loan_metadata_created_date,
...
uc.DATE_CAST(jsonb#>>'{loan,metadata,updatedDate}') AS loan_metadata_updated_date,
...
uc.DATE_CAST(jsonb#>>'{loan,agedToLostDelayedBilling,agedToLostDate}') AS loan_aged_to_lost_delayed_billing_aged_to_lost_date,
jsonb_pretty(jsonb) AS content
...
id AS id,
note_id AS note_id,
CAST(jsonb->>'id' AS UUID) AS object_id,
jsonb->>'type' AS type
...
uc.DATE_CAST(jsonb->>'dateOrdered') AS date_ordered,
...
uc.DATE_CAST(jsonb#>>'{ongoing,renewalDate}') AS ongoing_renewal_date,
uc.DATE_CAST(jsonb#>>'{ongoing,reviewDate}') AS ongoing_review_date,
...
uc.DATE_CAST(jsonb#>>'{eresource,expectedActivation}') AS eresource_expected_activation,
...
uc.DATE_CAST(jsonb#>>'{physical,expectedReceiptDate}') AS physical_expected_receipt_date,
...
jsonb->>'publisher' AS publisher,
CAST(jsonb->>'purchaseOrderId' AS UUID) AS order_id,
uc.DATE_CAST(jsonb->>'receiptDate') AS receipt_date,
jsonb->>'receiptStatus' AS receipt_status,
...
uc.DATE_CAST(jsonb#>>'{edi,ediJob,schedulingDate}') AS edi_job_scheduling_date,
...
jsonb->>'createdAt' AS service_point,
jsonb->>'source' AS source,
jsonb->>'paymentMethod' AS payment_method,
...
uc.DATE_CAST(jsonb->>'requestDate') AS request_date,
...
uc.DATE_CAST(jsonb->>'cancelledDate') AS cancelled_date,
...
uc.DATE_CAST(jsonb->>'requestExpirationDate') AS request_expiration_date,
uc.DATE_CAST(jsonb->>'holdShelfExpirationDate') AS hold_shelf_expiration_date,
...
CAST(jsonb#>>'{metadata,updatedByUserId}' AS UUID) AS updated_by_user_id,
jsonb#>>'{metadata,updatedByUsername}' AS updated_by_username,
uc.DATE_CAST(jsonb->>'awaitingPickupRequestClosedDate') AS awaiting_pickup_request_closed_date,
jsonb_pretty(COALESCE(jsonb_set(jsonb, '{metadata,createdDate}', ('"' || (jsonb#>>'{metadata,createdDate}') || CASE WHEN jsonb#>>'{metadata,createdDate}' !~ '([-+]\d\d:\d\d)|Z$' THEN '+00:00' ELSE '' END || '"')::jsonb), jsonb)) AS content
...
SELECT
id AS id,
uc.DATE_CAST(jsonb->>'expectedReceiptDate') AS expected_receipt_date,
jsonb->>'title' AS title,
CAST(jsonb->>'poLineId' AS UUID) AS po_line_id,
CAST(jsonb->>'instanceId' AS UUID) AS instance_id,
jsonb->>'publisher' AS publisher,
...
uc.DATE_CAST(jsonb#>>'{personal,dateOfBirth}') AS date_of_birth,
jsonb#>>'{personal,preferredContactTypeId}' AS preferred_contact_type_id,
uc.DATE_CAST(jsonb->>'enrollmentDate') AS enrollment_date,
uc.DATE_CAST(jsonb->>'expirationDate') AS expiration_date,
...
CAST(jsonb->>'loanId' AS UUID) AS loan_id,
uc.DATE_CAST(jsonb->>'dueDate') AS due_date,
...
INSERT INTO uc.payment_types (code, name, creation_time, creation_username, last_write_time, last_write_username) VALUES 
